...
// 获取文件的 Git 最后修改时间（与 VitePress 官方实现保持一致）
function getLastUpdated(filePath: string): number {
    try {
        // 使用与 VitePress 相同的 Git 命令格式
        const output = execSync(
            `git log -1 --pretty="%ai" "${filePath}"`,
            { encoding: 'utf-8' }
        ).trim()

        // 移除引号并解析时间
        const timeString = output.replace(/"/g, '')
        return new Date(timeString).getTime()
    } catch (error) {
        // 如果获取失败（比如文件未提交到 Git），返回文件的修改时间
        try {
